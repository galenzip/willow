...
From the tmux picker, `Ctrl-B` creates a stacked worktree from a picked base branch, and `Ctrl-E` opens the same existing-branch flow from cached refs first, then refreshes remote branches in the background so opening the picker does not wait on a remote fetch.
...
Generated detached worktrees like `detached-a13f09c` require an explicit branch name. When promoted, Willow moves their directory, agent status dir, and tmux session to the promoted branch identity. Explicitly named detached worktrees keep their existing directory and tmux session name.
...
Rename a worktree safely. Branch-backed worktrees rename the local branch and move the worktree directory. Detached worktrees only move the worktree directory. Agent status files and matching tmux sessions move with the worktree.
...
|------|-------------|---------|
| `-r, --repo` | Target repo by name | Auto-detected from cwd / all repos |
| `--remote` | Push the new branch and delete the old remote branch | `false` |
...
| `rename` | Worktree renamed via `ww rename` |
...
**Clickable notifications on macOS:** Install [`terminal-notifier`](https://github.com/julienXX/terminal-notifier) with `brew install terminal-notifier`. Clicking a built-in notification then switches the captured tmux session and brings its terminal forward in Ghostty 1.3+, iTerm2, or Terminal.app. Without `terminal-notifier`, Willow falls back to a plain `osascript` notification with no click action. Run `ww doctor` to check whether it is installed.

**Tabs outside tmux:** Supported terminals can select the matching tab when you enable `shell-init --tab-title` in your shell config. Without tab titles, or in another terminal, a click only brings the terminal app forward. For Ghostty, leave its `title` setting unset and add `shell-integration-features = no-title` to the Ghostty config so its own shell integration does not overwrite Willow's title. Ghostty 1.3+ also requires its [AppleScript integration](https://ghostty.org/docs/features/applescript), which is enabled by default with `macos-applescript = true`; macOS may ask for Automation access the first time.
...
The hook fires on 6 events (`UserPromptSubmit`, `PreToolUse`, `PostToolUse`, `Stop`, `Notification`, `SessionEnd`). Each invocation writes per-session status to `<willow-base>/status/<repo>/<worktree>/claude/<session_id>.json`, appends to the session timeline, and dispatches desktop notifications on status transitions. Multiple Claude sessions in the same worktree are tracked independently. The `SessionEnd` event immediately removes the session file for instant cleanup.

### `ww codex-setup`

Hook installation for Codex CLI status tracking. Safe to re-run: each invocation overwrites Willow's entries in `~/.codex/hooks.json` with the current binary path and leaves unrelated hook rules untouched. Codex may ask you to review and trust the hook with `/hooks`.
...
The command rejects overlapping paths, non-empty destinations, active agent session files, and running from inside the current willow base.
...
✔ gh CLI installed
✔ tmux installed
✔ terminal-notifier installed (clickable notifications)
✔ Claude Code (default) CLI installed
...
| terminal-notifier | macOS only; optional dependency for notification clicks that focus the agent session |
